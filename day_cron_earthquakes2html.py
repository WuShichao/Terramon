...
def start_html_file(title, filename):
    ...
    os.system('rm /home/hunter.gabbard/public_html/earthquake_mon/daily_veto_segments/tmp/veto_segs.txt')
    file.write('<html>')
    file.write('<head><title>Earthquake data: </title></head>')
    file.write('<body>')
    file.write('<h4 style="position: absolute; z-index: -1; top: 0px; right: 15px;">Last refresh: <script>document.write(document.lastModified)</script></h4>')
...
def process_sub_dir(dir, file):
    ...
    seg = open('/home/hunter.gabbard/public_html/earthquake_mon/daily_veto_segments/tmp/veto_segs.txt',  'a')
    tp = 0
    tRf = 0
    #store values
    a = open(efilename,"r")
	...
	for i in range(len(c1)):
		...
		#seg.write('Location: %s; LAT: %s, LON: %s\n' % (place, c1[i], c1[i]))
	for i in range(len(c1)):
	    if i == 1:
		file.write('<h3>Magnitude: %s</h3>' % c1[i])
		#seg.write('Magnitude: %s\n' % c1[i])
	...
	for i in range(len(c1)):
		...
		PDT_time1 = float(c1[i]) - 28800
                ...
                file.write('<td> %s PT </td>' % hanford_title1)
		...
		PDT_time2 = float(c1[i]) - 28800
                ...
                file.write('<td> %s PT </td>' % hanford_title2)

	#for i in range(len(c1)):
	    #if i == 0:
		#PDT_time3 = float(c1[i]) - 25200
                #PDT_time3 = int(PDT_time3)
                #Hanford_title3 = commands.getstatusoutput("lalapps_tconvert --local --zone=PDT -f%%H:%%M:%%S %d" % (PDT_time3))
                #han_title3 = str(Hanford_title3)
                #hanford3 = han_title3.replace(han_title3[:5], '')
                #hanford_title3 = hanford3.replace(hanford3[8:], '')
                #file.write('<td> %s PDT </td>' % hanford_title3)
	...
	for i in range(len(c1)):
		...
		PDT_time4 = float(c1[i]) - 28800
                ...
                file.write('<td><b> %s PT </b></td>' % hanford_title4)
	...
	for i in range(len(c2)):
		...
		CST_time1 = float(c2[i]) - 21600
                ...
                file.write('<td> %s CT</td>' % livingston_title1)
		...
		CST_time2 = float(c2[i]) - 21600
                ...
                file.write('<td> %s CT</td>' % livingston_title2)
		...
		CST_time4 = float(c2[i]) - 21600
                ...
                file.write('<td><b> %s CT</b></td>' % livingston_title4)
	...
	#for i in range(len(c4)):
            #if i == 5:
                #CESTI_time = float(c4[i]) + 7200
                #CESTI_time = int(CESTI_time)
                #Pisa_title = commands.getstatusoutput("lalapps_tconvert --local --zone=PDT -f%%H:%%M:%%S %d" % (CESTI_time))
                #P_title = str(Pisa_title)
                #pisa = P_title.replace(P_title[:5], '')
                #pisa_title = pisa.replace(pisa[8:], '')
                #file.write('<td> %s CEST </td>' % pisa_title)
