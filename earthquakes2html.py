...
def start_html_file(title, filename):
    ...
    #/users/Miyo/public_html/earthquake_mon 
    os.system('rm /users/Miyo/public_html/earthquake_mon/veto_segs.txt')
    file.write('<html>')
...
def process_sub_dir(dir, file):
    '''
     0 attributeDic["eventID"],
     1 attributeDic["GPS"],
     2 attributeDic["Magnitude"],
     3 max(traveltimes["Ptimes"]),
     4 max(traveltimes["Stimes"]),
     5 max(traveltimes["Rtwotimes"]),
     6 max(traveltimes["RthreePointFivetimes"]),
     7 max(traveltimes["Rfivetimes"]),
     8 traveltimes["Rfamp"][0],
     9 traveltimes["Rfamp_sigma"][0],
    10 arrival_floor,
    11 departure_ceil,
    12 attributeDic["Latitude"],
    13 attributeDic["Longitude"],
    14 max(traveltimes["Distances"]),
    15 attributeDic["Depth"],
    16 traveltimes["Azimuth"][0],
    17 traveltimes["Lockloss"][0],
    18 traveltimes["Lockloss_sigma"][0],
    19 locationstr,
    20 attributeDic["SentGPS"],
    21 ifoShort
    '''    
    # Attempt to open earthquakes.txt in dir
    efilename = os.path.join(dir, 'earthquakes.txt')
    seg = open('/users/Miyo/public_html/earthquake_mon/veto_segs.txt',  'a')
    tp = 0
    tRf = 0
    #Store values
    a = open(efilename,"r")
    ...
    try:
        ...
        New_title = subprocess.getstatusoutput("lalapps_tconvert --local --zone=UTC %d" % (integer_title))
	
        link_time = subprocess.getstatusoutput("lalapps_tconvert --local --zone=UTC -f%%Y-%%m-%%d-%%H-%%M-%%S %d" % (integer_title))
	...
	#Getting csv files for human readable location
        os.system("rm /users/Miyo/public_html/earthquake_mon/event_csvs/event.csv")
        os.system("wget '%s' -O /users/Miyo/public_html/earthquake_mon/event_csvs/event.csv" % web_link)
        try:
            with open('/users/Miyo/public_html/earthquake_mon/event_csvs/event.csv', 'rb') as csvfile:
    ...
    for i in range(len(c1)):
        if i == 3:
            ...
            Hanford_title1 = subprocess.getstatusoutput("lalapps_tconvert --local --zone=PDT -f%%H:%%M:%%S %d" % (PDT_time1))
    ...
    for i in range(len(c1)):
        if i == 4:
            ...
            Hanford_title2 = subprocess.getstatusoutput("lalapps_tconvert --local --zone=PDT -f%%H:%%M:%%S %d" % (PDT_time2))
    ...
    for i in range(len(c1)):
        if i == 5:
            ...
            Hanford_title4 = subprocess.getstatusoutput("lalapps_tconvert --local --zone=PDT -f%%H:%%M:%%S %d" % (PDT_time4))
    ...
    for i in range(len(c2)):
        if i == 3:
            ...
            Livingston_title1 = subprocess.getstatusoutput("lalapps_tconvert --local --zone=PDT -f%%H:%%M:%%S %d" % (CST_time1))
    ...
    for i in range(len(c2)):
        if i == 4:
            ...
            Livingston_title2 = subprocess.getstatusoutput("lalapps_tconvert --local --zone=PDT -f%%H:%%M:%%S %d" % (CST_time2))
    ...
    for i in range(len(c2)):
        if i == 5:
            ...
            Livingston_title4 = subprocess.getstatusoutput("lalapps_tconvert --local --zone=PDT -f%%H:%%M:%%S %d" % (CST_time4))
    ...
    for i in range(len(c2)):
        ...
        for i in range(len(c3)):
            if i == 3:
                ...
                Geo_title1 = subprocess.getstatusoutput("lalapps_tconvert --local --zone=PDT -f%%H:%%M:%%S %d" % (CEST_time1))
        ...
        for i in range(len(c3)):
            if i == 4:
                ...
                Geo_title2 = subprocess.getstatusoutput("lalapps_tconvert --local --zone=PDT -f%%H:%%M:%%S %d" % (CEST_time2))
        ...
        for i in range(len(c3)):
            if i == 5:
                ...
                Geo_title4 = subprocess.getstatusoutput("lalapps_tconvert --local --zone=PDT -f%%H:%%M:%%S %d" % (CEST_time4))
    ...
    for i in range(len(c4)):
        ...
        for i in range(len(c4)):
            if i == 3:
                ...
                Pisa_title1 = subprocess.getstatusoutput("lalapps_tconvert --local --zone=PDT -f%%H:%%M:%%S %d" % (CESTI_time1))
        ...
        for i in range(len(c4)):
            if i == 4:
                ...
                Pisa_title2 = subprocess.getstatusoutput("lalapps_tconvert --local --zone=PDT -f%%H:%%M:%%S %d" % (CESTI_time2))
        ...
        for i in range(len(c4)):
            if i == 5:
                ...
                Pisa_title4 = subprocess.getstatusoutput("lalapps_tconvert --local --zone=PDT -f%%H:%%M:%%S %d" % (CESTI_time4))
...
def main():
    ...
    segment = "/users/Miyo/public_html/earthquake_mon/veto_segments.txt" 
    ...
    # Start HTML file
    file = start_html_file(sys.argv[1],  "/users/Miyo/public_html/earthquake_mon/html_pages/earthquakes.html")
